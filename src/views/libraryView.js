...
function renderDetailLifecycleActions(collection, item) {
  return `
    <div class="inline-actions" style="margin-top:0.4rem;">
      <button class="inline-button btn-secondary" type="button" data-archive-entity="${collection}" data-id="${item.id}">${isArchived(item) ? 'Unarchive' : 'Archive'}</button>
      <button class="inline-button btn-danger" type="button" data-request-delete="${collection}" data-id="${item.id}" data-delete-mode="soft">Request delete</button>
      <button class="inline-button btn-danger" type="button" data-request-delete="${collection}" data-id="${item.id}" data-delete-mode="hard" data-delete-scope="detail">Hard delete</button>
    </div>
  `;
}
  ...
  if (collection === 'tasks') {
    return `
      <form data-library-entity-form data-collection="tasks" data-id="${selected.id}" class="library-edit-form" aria-label="Task detail edit form">
        ...
        <div class="library-edit-actions"><button class="button btn-primary" type="submit">Save task</button></div>
      </form>
    `;
  }
  ...
  if (collection === 'projects') {
    return `
      <form data-library-entity-form data-collection="projects" data-id="${selected.id}" class="library-edit-form" aria-label="Project detail edit form">
        ...
        <div class="library-edit-actions"><button class="button btn-primary" type="submit">Save project</button></div>
      </form>
    `;
  }
  ...
  if (collection === 'reminders') {
    return `
      <form data-library-entity-form data-collection="reminders" data-id="${selected.id}" class="library-edit-form" aria-label="Reminder detail edit form">
        ...
        <div class="library-edit-row"><label class="muted" for="${idPrefix}-priority">Priority</label><input id="${idPrefix}-priority" class="input" name="priority" type="number" min="1" max="5" value="${escapeHtml(String(selected.priority || 3))}" /></div>
        <div class="library-edit-row"><label class="muted" for="${idPrefix}-context">Context</label><input id="${idPrefix}-context" class="input" name="context" value="${escapeHtml(selected.context || 'work')}" /></div>
        <div class="library-edit-actions"><button class="button btn-primary" type="submit">Save reminder</button></div>
      </form>
    `;
  }
    ...
    <div class="cols">
      <section class="col">
        ...
        <div class="row-list" style="margin-top:0.35rem;">
          ${rows.map((entry) => `<a class="mode-link ${selected?.id === entry.id ? 'active' : ''}" href="#/library/${collection}/${entry.id}">${escapeHtml(entry[titleField] || entry.name || entry.id)}</a>`).join('') || '<p class="empty-state">No active items.</p>'}
        </div>
      </section>
      <section class="col">
        ...
        ` : '<p class="empty-state">No item selected.</p>'}
      </section>
    </div>
  `;
}
      ...
      <section class="col">
        ...
        ${selected ? `
          <form data-library-entity-form data-collection="people" data-id="${selected.id}" class="library-edit-form" aria-label="Person detail edit form">
            ...
            <div class="library-edit-actions"><button class="button btn-primary" type="submit">Save person</button></div>
          </form>
          <h4>Pending updates</h4>
          <div class="row-list">${pending.map((entry) => `<article class="row">${escapeHtml(entry)}</article>`).join('') || '<p class="empty-state">No pending updates.</p>'}</div>
          ${renderDetailLifecycleActions('people', selected)}
        ` : '<p class="empty-state">No person selected.</p>'}
      </section>
    </div>
  `;
}
        ...
        ${selected ? `
          <form data-library-meeting-form data-id="${selected.id}" class="library-edit-form" aria-label="Meeting detail edit form">
            ...
            <div class="library-edit-actions">
              <button class="button btn-primary" type="submit">Save meeting</button>
            </div>
          </form>
          <div class="row-list" style="margin-top:0.4rem;">${groups.map((group) => `<article class="row"><strong>${escapeHtml(group.title)}</strong></article>`).join('') || '<p class="empty-state">No follow-ups.</p>'}</div>
          ${renderDetailLifecycleActions('meetings', selected)}
        ` : '<p class="empty-state">No meeting selected.</p>'}
      </section>
    </div>
  `;
}
...
function renderArchivedOrDeleted(state, mode = 'archived') {
          ...
          ${rows.map((item) => `
            <article class="row">
              ...
              <div class="inline-actions">
                <button class="inline-button btn-secondary" type="button" data-restore-entity="${collection}" data-id="${item.id}">Restore</button>
              </div>
            </article>
          `).join('')}
        </div>
  ...
  return blocks || `<p class="empty-state">No recently ${mode} items.</p>`;
}

function renderHabits(state) {
  const habits = Array.isArray(state.habits) ? state.habits : [];
  if (!habits.length) return '<p class="empty-state">No habits tracked yet.</p>';
...
function renderLogs(state) {
  if (!state.dailyLogs?.length) return '<p class="empty-state">No Daily Logs yet.</p>';
...
export function renderLibrary(state, routeParts) {
  ...
  let body = '<p class="empty-state">Select a library section.</p>';
  ...
  return `
    <div class="library-grid">
      ...
      <section>
        <div class="view-header"><h2>Library / ${titleCase(section)}</h2><span class="chip badge-accent">${titleCase(section)}</span></div>
